...
export default function CustomersPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6 w-full">
      ...
      {/* Mobile Customers Cards List (< 640px) */}
            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="text-right">
                  <span className="text-[10px] font-semibold text-slate-400 block uppercase">Due Balance</span>
                  {cust.creditBalance > 0 ? (
                    <span className="text-sm font-extrabold text-amber-700 bg-amber-50 px-2 py-0.5 rounded tabular-nums">
                      {formatCurrency(cust.creditBalance)}
                    </span>
                  ) : (
                    <span className="text-xs text-emerald-700 font-semibold">₹0.00</span>
                  )}
                </div>
              </div>

              {(cust.state || cust.address || cust.gstin) && (
                <div className="text-xs text-slate-500 space-y-0.5 bg-slate-50 p-2.5 rounded-lg">
                  {cust.address && <p className="truncate">{cust.address}, {cust.state || "Delhi"}</p>}
                  {cust.gstin && (
                    <p className="font-mono text-[11px] text-slate-700">
                      GSTIN: <span className="font-semibold">{cust.gstin}</span>
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="flex items-center justify-between pt-1">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-teal-50 hover:bg-teal-100 text-teal-800 rounded-lg text-xs font-bold transition"
                >
